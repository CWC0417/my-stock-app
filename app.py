...
# ===================================================
# ☁️ 2. Google Sheets 雲端資料讀寫核心工具
# ===================================================
conn = st.connection("gsheets", type=GSheetsConnection)

def load_sheet_to_dict(worksheet_name, default_data):
    """從 Google Sheets 讀取資料並完美轉換為系統原本使用的字典格式"""
    try:
        ...
        # 針對名稱庫(names_db)進行扁平化處理，維持相容性
        if worksheet_name == "names_db":
            return {k: v.get('name', k) for k, v in res_dict.items()}
...
def save_dict_to_sheet(worksheet_name, data_dict):
    """將原本系統的字典格式自動重組為 DataFrame 並寫入 Google Sheets"""
    try:
        if worksheet_name == "names_db":
...
# ===================================================
# 🚀 3. FinMind 核心數據引擎 (極速快取版)
# ===================================================
@st.cache_data(ttl=3600)  # 快取 1 小時，大幅節省 API 額度並防止被 API 阻擋鎖定
def fetch_clean_stock_data(ticker_symbol, token):
    # 強效清理代碼 (FinMind 只需要數字，徹底去除 .TW, .TWO 避免抓不到資料)
    clean_id = ticker_symbol.replace(".TW", "").replace(".TWO", "").replace(".v", "").replace(".V", "")
    ...
    try:
        ...
        if price_data.get("msg") != "success" or not price_data.get("data"):
            return None, {}, f"代碼 [{clean_id}] 不存在或已達今日 API 上限。"
...
st.caption("🚀 全面導入 FinMind API │ 🛡️ 整合 Google Sheets 雲端永久儲存 │ 👑 穩定高速版")
...
with main_tab:
    if not st.session_state.watchlist:
        ...
        st.error("🚨 系統偵測到你尚未設定 FinMind Token！")
    else:
...
with control_tab:
    ...
    with col_bak2:
        st.write("② 滿血復活：將上傳的舊 JSON 資料同步覆寫回 Google Sheets")
        ...
        if uploaded_backup is not None:
            try:
                ...
                if "watchlist" in uploaded_data:
                    ...
                    names_db = uploaded_data.get("names", {})
                    backup_db = uploaded_data.get("backup_db", {})
                    
                    # 覆寫儲存回 Google Sheets 雲端
                    ...
                    st.success("✨ 雲端資料同步還原成功！網頁即將重整...")
                    time.sleep(1.0)
                    st.rerun()
            except Exception as ex: 
                st.error(f"❌ 解析或寫入雲端失敗: {str(ex)}")
        ...
        if st.button("💾 確認儲存股票", use_container_width=True):
            if new_stock:
                ...
                if custom_name: 
                    ...
                    save_dict_to_sheet("names_db", names_db)
                st.cache_data.clear()
                st.success(f"股票 {new_stock} 儲存並成功同步至雲端！")
                time.sleep(0.5)
                st.rerun()
    ...
    with col_right:
        ...
        if st.session_state.watchlist:
            ...
            if st.button("💾 儲存並同步到卡片", use_container_width=True, key=f"save_btn_{tgt_b}"):
                ...
                # 更新回雲端 backup_db 工作表
                save_dict_to_sheet("backup_db", backup_db)
                st.success(f"✨ {tgt_b} 專屬數據同步至雲端成功！")
                time.sleep(0.5)
                st.rerun()
        else:
            st.info("請先在左側新增股票。")