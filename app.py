...
if not st.session_state.authenticated:
    st.markdown("<h3 style='text-align: center; margin-top: 50px;'>🔒 歡迎來到個人看盤戰情室 (v15.3)</h3>", unsafe_allow_html=True)
    st.write("---")
...
# --- 介面 ---
st.title("📊 個人化智慧看盤系統 v15.3")
st.caption("🤖 Gooaye 投資哲學量化版 │ 🔎 修正籌碼欄位未顯示問題 │ 數據自動防錯備援機制")
...
with main_tab:
    if not st.session_state.watchlist:
        ...
        with col_ctrl1: view_mode = st.radio("顯示模式", ["📱 手機卡片 (推薦)", "📋 完整表格"], horizontal=True, key="v_mode_153")
        with col_ctrl2: ma_strategy = st.radio("買點策略", ["波段操作 (20MA)", "長線大底 (60MA)"], horizontal=True, key="ma_strat_153")
        ...
        for ticker_symbol, item in st.session_state.watchlist.items():
            ...
            rows.append({
                ...
                "👤籌碼": chips_status, "🚨決策": hold_action, "損益": pnl_str, "數據來源": mode_tag
            })
...
with control_tab:
    ...
    with col_left:
        ...
        new_stock = st.text_input("股票代碼 (例: 2330.TW)", key="add_id_v153").upper().strip()
        custom_name = st.text_input("股票中文別名 (例: 台積電)", key="add_name_v153")
        stock_type = st.selectbox("類別", ["觀察中 (尚未買進)", "已持股"], key="add_type_v153")
        cost, qty = 0.0, 0
        if stock_type == "已持股":
            cost = st.number_input("買入成本價", min_value=0.0, step=0.1, key="add_cost_v153")
            qty = st.number_input("持有股數", min_value=0, step=100, key="add_qty_v153")
            
        if st.button("💾 確認儲存股票", use_container_width=True, key="save_btn_v153"):
            if new_stock:
        ...
        if st.session_state.watchlist:
            tgt_p = st.selectbox("選擇股票點選現價", list(st.session_state.watchlist.keys()), key="price_tgt_v153")
            input_p = st.number_input(f"輸入 {tgt_p} 即時價", min_value=0.0, step=0.1, key="price_val_v153")
            if st.button("⚡ 同步現價", use_container_width=True, key="sync_btn_v153"):
                if input_p > 0:
    ...
    with col_right:
        ...
        if st.session_state.watchlist:
            del_target = st.selectbox("選擇你想刪除的股票", list(st.session_state.watchlist.keys()), key="del_box_v153")
            if st.button("🔥 確定完全刪除", type="primary", use_container_width=True, key="del_btn_v153"):
        ...
        if st.session_state.watchlist:
            tgt_b = st.selectbox("選擇要備援的股票", list(st.session_state.watchlist.keys()), key="sb_box_v153")
            cur_b = backup_db.get(tgt_b, {"net_buy_5d": 0, "rev_6ma": 1.0, "rev_12ma": 0.0, "pe": 15.0, "yield": 4.5})
            
            pe_in = st.number_input("手動本益比 (PE)", value=float(cur_b.get("pe", 15.0)), step=1.0, key="b_pe_v153")
            y_in = st.number_input("手動殖利率 (%)", value=float(cur_b.get("yield", 4.5)), step=0.1, key="b_yield_v153")
            chip_in = st.number_input("近 5 日法人累積買超 (張)", value=int(cur_b.get("net_buy_5d", 0)), step=100, key="b_chip_v153")
            
            st.markdown("**營收趨勢判定：6MA > 12MA 就會亮綠燈**")
            r6 = st.number_input("6MA 營收水位", value=float(cur_b.get("rev_6ma", 1.0)), step=1.0, key="b_r6_v153")
            r12 = st.number_input("12MA 營收水位", value=float(cur_b.get("rev_12ma", 0.0)), step=1.0, key="b_r12_v153")
            
            if st.button("💾 儲存備援數據", use_container_width=True, key="b_btn_v153"):
